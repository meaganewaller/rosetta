...
- [~] Author/curate plugins per category; each passes the validator and has adapter coverage.
      **14 plugins across 12 categories so far** — the Languages kickoff (`python-idioms`,
      `typescript-idioms`, `ruby-idioms`) and the AI & ML cluster (`prompt-engineering`,
      `llm-evals`, `agent-memory`), all authored with the dogfooding toolkit. Live count:
      [`docs/coverage.md`](docs/coverage.md).
- [x] Track coverage publicly — [`docs/coverage.md`](docs/coverage.md), generated by
      `mise run coverage` (category coverage + per-plugin × harness fidelity) and kept fresh by CI.
